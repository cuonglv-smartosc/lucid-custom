...
const mod_js_1 = require("../utils/mod.js");
const utils_js_1 = require("../utils/utils.js");
class Emulator {
    constructor(accounts, protocolParameters = mod_js_1.PROTOCOL_PARAMETERS_DEFAULT) {
            ...
            this.ledger[GENESIS_HASH + index] = {
                utxo: {
                    ...
                    datumHash: outputData?.asHash
                        ? core_js_1.C.hash_plutus_data(core_js_1.C.PlutusData.from_bytes((0, utils_js_1.fromHex)(outputData.asHash))).to_hex()
                        : outputData?.hash,
    ...
    getUtxos(addressOrCredential) {
        const utxos = Object.values(this.ledger).flatMap(({ utxo }) => {
            if (typeof addressOrCredential === "string") {
                ...
                const { paymentCredential } = (0, utils_js_1.getAddressDetails)(utxo.address);
    ...
    submitTx(tx) {
        /*
            ...
            Checks that need to be done:
              ...
              - Validity interval
         */
        const desTx = core_js_1.C.Transaction.from_bytes((0, utils_js_1.fromHex)(tx));
        const body = desTx.body();
        const witnesses = desTx.witness_set();
        const datums = witnesses.plutus_data();
        const txHash = core_js_1.C.hash_transaction(body).to_hex();
        // Validity interval
        ...
        const datumTable = (() => {
            ...
            for (let i = 0; i < (datums?.len() || 0); i++) {
                ...
                const datumHash = core_js_1.C.hash_plutus_data(datum).to_hex();
                table[datumHash] = (0, utils_js_1.toHex)(datum.to_bytes());
            }
            return table;
        })();
        ...
        const keyHashes = (() => {
            ...
            for (let i = 0; i < (witnesses.vkeys()?.len() || 0); i++) {
                ...
                if (!publicKey.verify((0, utils_js_1.fromHex)(txHash), witness.signature())) {
        ...
        const edKeyHashes = core_js_1.C.Ed25519KeyHashes.new();
        keyHashes.forEach((keyHash) => edKeyHashes.add(core_js_1.C.Ed25519KeyHash.from_hex(keyHash)));
        const nativeHashes = (() => {
            const scriptHashes = [];
            for (let i = 0; i < (witnesses.native_scripts()?.len() || 0); i++) {
                const witness = witnesses.native_scripts().get(i);
                const scriptHash = witness.hash(core_js_1.C.ScriptHashNamespace.NativeScript)
                    .to_hex();
                if (!witness.verify(Number.isInteger(lowerBound)
                    ? core_js_1.C.BigNum.from_str(lowerBound.toString())
                    : undefined, Number.isInteger(upperBound)
                    ? core_js_1.C.BigNum.from_str(upperBound.toString())
        ...
        const plutusHashes = (() => {
            ...
            for (let i = 0; i < (witnesses.plutus_scripts()?.len() || 0); i++) {
                ...
                const scriptHash = script.hash(core_js_1.C.ScriptHashNamespace.PlutusV1)
                    .to_hex();
                scriptHashes.push(scriptHash);
            }
            for (let i = 0; i < (witnesses.plutus_v2_scripts()?.len() || 0); i++) {
                const script = witnesses.plutus_v2_scripts().get(i);
                const scriptHash = script.hash(core_js_1.C.ScriptHashNamespace.PlutusV2)
                    .to_hex();
                scriptHashes.push(scriptHash);
            }
            return scriptHashes;
        ...
        for (let i = 0; i < inputs.len(); i++) {
            ...
            if (scriptRef) {
                switch (scriptRef.type) {
                    case "Native": {
                        const script = core_js_1.C.NativeScript.from_bytes((0, utils_js_1.fromHex)(scriptRef.script));
                        nativeHashesOptional[script.hash(core_js_1.C.ScriptHashNamespace.NativeScript).to_hex()] = script;
                        break;
                    }
                    case "PlutusV1": {
                        const script = core_js_1.C.PlutusScript.from_bytes((0, utils_js_1.fromHex)(scriptRef.script));
                        plutusHashesOptional.push(script.hash(core_js_1.C.ScriptHashNamespace.PlutusV1).to_hex());
                        break;
                    }
                    case "PlutusV2": {
                        const script = core_js_1.C.PlutusScript.from_bytes((0, utils_js_1.fromHex)(scriptRef.script));
                        plutusHashesOptional.push(script.hash(core_js_1.C.ScriptHashNamespace.PlutusV2).to_hex());
                        break;
                    }
                }
            }
        ...
        for (let i = 0; i < (body.collateral()?.len() || 0); i++) {
            ...
            const { paymentCredential } = (0, utils_js_1.getAddressDetails)(entry.utxo.address);
        ...
        for (let index = 0; index < (body.withdrawals()?.keys().len() || 0); index++) {
            ...
            const { stakeCredential } = (0, utils_js_1.getAddressDetails)(rewardAddress);
            ...
            */
            ...
            switch (cert.kind()) {
                case 0: {
                    ...
                    const rewardAddress = core_js_1.C.RewardAddress.new(core_js_1.C.NetworkInfo.testnet().network_id(), registration.stake_credential()).to_address().to_bech32(undefined);
                ...
                case 1: {
                    ...
                    const rewardAddress = core_js_1.C.RewardAddress.new(core_js_1.C.NetworkInfo.testnet().network_id(), deregistration.stake_credential()).to_address().to_bech32(undefined);
                    const { stakeCredential } = (0, utils_js_1.getAddressDetails)(rewardAddress);
                ...
                case 2: {
                    ...
                    const rewardAddress = core_js_1.C.RewardAddress.new(core_js_1.C.NetworkInfo.testnet().network_id(), delegation.stake_credential()).to_address().to_bech32(undefined);
        ...
        const outputs = (() => {
            ...
            for (let i = 0; i < body.outputs().len(); i++) {
                ...
                const unspentOutput = core_js_1.C.TransactionUnspentOutput.new(core_js_1.C.TransactionInput.new(core_js_1.C.TransactionHash.from_hex(txHash), core_js_1.C.BigNum.from_str(i.toString())), output);
                const utxo = (0, utils_js_1.coreToUtxo)(unspentOutput);
                if (utxo.datumHash)
