...
exports.discoverOwnUsedTxKeyHashes = exports.walletFromSeed = void 0;
...
function walletFromSeed(seed, options = { addressType: "Base", accountIndex: 0, network: "Mainnet" }) {
    ...
    const entropy = (0, bip39_js_1.mnemonicToEntropy)(seed);
    const rootKey = mod_js_1.C.Bip32PrivateKey.from_bip39_entropy((0, mod_js_1.fromHex)(entropy), options.password
    ...
    const address = options.addressType === "Base"
        ? mod_js_1.C.BaseAddress.new(networkId, mod_js_1.C.StakeCredential.from_keyhash(paymentKeyHash), mod_js_1.C.StakeCredential.from_keyhash(stakeKeyHash)).to_address().to_bech32(undefined)
        : mod_js_1.C.EnterpriseAddress.new(networkId, mod_js_1.C.StakeCredential.from_keyhash(paymentKeyHash)).to_address().to_bech32(undefined);
    const rewardAddress = options.addressType === "Base"
        ? mod_js_1.C.RewardAddress.new(networkId, mod_js_1.C.StakeCredential.from_keyhash(stakeKeyHash)).to_address().to_bech32(undefined)
        : null;
    return {
        address,
        rewardAddress,
...
function discoverOwnUsedTxKeyHashes(tx, ownKeyHashes, ownUtxos) {
    ...
    for (let i = 0; i < inputs.len(); i++) {
        ...
        const txHash = (0, mod_js_1.toHex)(input.transaction_id().to_bytes());
        ...
        if (utxo) {
            const { paymentCredential } = (0, mod_js_1.getAddressDetails)(utxo.address);
    ...
    function keyHashFromCert(txBody) {
        ...
        for (let i = 0; i < certs.len(); i++) {
            ...
            if (cert.kind() === 0) {
                ...
                if (credential?.kind() === 0) {
                    const keyHash = (0, mod_js_1.toHex)(credential.to_keyhash().to_bytes());
                ...
                for (let i = 0; i < owners.len(); i++) {
                    const keyHash = (0, mod_js_1.toHex)(owners.get(i).to_bytes());
                    usedKeyHashes.push(keyHash);
                }
    ...
    function keyHashFromScript(scripts) {
        for (let i = 0; i < scripts.len(); i++) {
            ...
            if (script.kind() === 0) {
                const keyHash = (0, mod_js_1.toHex)(script.as_script_pubkey().addr_keyhash().to_bytes());
                usedKeyHashes.push(keyHash);
            }
            if (script.kind() === 1) {
    ...
    if (requiredSigners) {
        for (let i = 0; i < requiredSigners.len(); i++) {
            usedKeyHashes.push((0, mod_js_1.toHex)(requiredSigners.get(i).to_bytes()));
        }
    }
