...
const typebox_js_1 = require("../../deps/deno.land/x/typebox@0.25.13/src/typebox.js");
...
exports.Data = {
    ...
    Integer: function (options) {
        const integer = typebox_js_1.Type.Unsafe({ dataType: "integer" });
        if (options) {
    ...
    Bytes: function (options) {
        const bytes = typebox_js_1.Type.Unsafe({ dataType: "bytes" });
        if (options) {
            Object.entries(options).forEach(([key, value]) => {
                bytes[key] = value;
            });
    ...
    Boolean: function () {
        return typebox_js_1.Type.Unsafe({
            anyOf: [
                {
                    title: "False",
    ...
    Any: function () {
        return typebox_js_1.Type.Unsafe({ description: "Any Data." });
    },
    Array: function (items, options) {
        const array = typebox_js_1.Type.Array(items);
    ...
    Map: function (keys, values, options) {
        const map = typebox_js_1.Type.Unsafe({
            dataType: "map",
            keys,
            values,
        });
    ...
    Object: function (properties, options) {
        const object = typebox_js_1.Type.Object(properties);
    ...
    Enum: function (items) {
        const union = typebox_js_1.Type.Union(items);
    ...
    Tuple: function (items, options) {
        const tuple = typebox_js_1.Type.Tuple(items);
    ...
    Literal: function (title) {
        ...
        const literal = typebox_js_1.Type.Literal(title);
...
function to(data, type) {
    function serialize(data) {
        try {
            if (typeof data === "bigint") {
                return mod_js_1.C.PlutusData.new_integer(mod_js_1.C.BigInt.from_str(data.toString()));
            }
            else if (typeof data === "string") {
                return mod_js_1.C.PlutusData.new_bytes((0, utils_js_1.fromHex)(data));
            }
            else if (data instanceof Constr) {
                const { index, fields } = data;
                const plutusList = mod_js_1.C.PlutusList.new();
                fields.forEach((field) => plutusList.add(serialize(field)));
                return mod_js_1.C.PlutusData.new_constr_plutus_data(mod_js_1.C.ConstrPlutusData.new(mod_js_1.C.BigNum.from_str(index.toString()), plutusList));
            }
            else if (data instanceof Array) {
                const plutusList = mod_js_1.C.PlutusList.new();
                data.forEach((arg) => plutusList.add(serialize(arg)));
                return mod_js_1.C.PlutusData.new_list(plutusList);
            }
            else if (data instanceof Map) {
                const plutusMap = mod_js_1.C.PlutusMap.new();
                for (const [key, value] of data.entries()) {
                    plutusMap.insert(serialize(key), serialize(value));
                }
                return mod_js_1.C.PlutusData.new_map(plutusMap);
            }
    ...
    return (0, utils_js_1.toHex)(serialize(d).to_bytes());
}
/**
...
function from(raw, type) {
    function deserialize(data) {
        if (data.kind() === 0) {
            ...
            return (0, utils_js_1.toHex)(data.as_bytes());
        }
        throw new Error("Unsupported type");
    }
    const data = deserialize(mod_js_1.C.PlutusData.from_bytes((0, utils_js_1.fromHex)(raw)));
    ...
    function toData(json) {
        if (typeof json === "string") {
            return json.startsWith("0x")
                ? (0, utils_js_1.toHex)((0, utils_js_1.fromHex)(json.slice(2)))
                : (0, utils_js_1.fromText)(json);
        }
...
function toJson(plutusData) {
    function fromData(data) {
        ...
        if (typeof data === "string") {
            try {
                return new TextDecoder(undefined, { fatal: true }).decode((0, utils_js_1.fromHex)(data));
            }
            catch (_) {
                return "0x" + (0, utils_js_1.toHex)((0, utils_js_1.fromHex)(data));
            }
        }
